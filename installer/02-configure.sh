...
# If LUKS encryption was requested in stage 1, verify the mapped root is
# present and record the raw LUKS partition UUID (needed by the boot module's
# boot.initrd.luks.devices.<mapper>.device).
if is_luks; then
    section "LUKS ENCRYPTION"
    [[ -n "$LUKS_PART" ]] || die "LUKS was requested in stage 1 but no LUKS_PART was recorded."
    luks_type="$(blkid -s TYPE -o value "$LUKS_PART" 2>/dev/null || true)"
    if [[ "$luks_type" == "crypto_LUKS" ]]; then
        echo "  $LUKS_PART is a LUKS2 container (TYPE=crypto_LUKS)."
    else
        warn "$LUKS_PART has TYPE='${luks_type:-?}' — expected crypto_LUKS. Was it set up by stage 1?"
    fi
    mapper="/dev/mapper/$LUKS_MAPPER"
    if [[ -e "$mapper" ]] && findmnt -n -o FSTYPE "$TARGET" 2>/dev/null | grep -q btrfs; then
        echo "  encrypted root is open and mounted as Btrfs at $mapper."
    else
        warn "encrypted root not mounted at $mapper — run stage 1 again."
    fi
    luks_uuid="$(uuid_of "$LUKS_PART")"
    if [[ -n "$luks_uuid" ]]; then
        LUKS_UUID="$luks_uuid"
        state_save TARGET HOST EFI_PART ROOT_PART SWAP_PART LUKS LUKS_PART LUKS_MAPPER LUKS_UUID
        echo "  LUKS container UUID: $luks_uuid"
        echo "  (Use this UUID in boot.initrd.luks.devices.\"$LUKS_MAPPER\".device)"

        # Generate the host-specific LUKS device declaration so the target
        # config can actually boot: without boot.initrd.luks.devices."<mapper>"
        # the initrd has nothing to decrypt, so an encrypted root won't come up
        # even with the right passphrase. The host policy imports this file
        # conditionally (builtins.pathExists), so unencrypted hosts are
        # unaffected when it's absent.
        LUKS_HW="$TARGET_FLAKE/hosts/$HOST/luks-configuration.nix"
        mkdir -p "$TARGET_FLAKE/hosts/$HOST"
        cat > "$LUKS_HW" <<EOF
# Generated by the installer (stage 2) on $(date --iso-8601=seconds).
# Full-disk LUKS2 encryption declaration for $HOST.
...
{ config, lib, ... }:
{
  boot.initrd.luks.devices."$LUKS_MAPPER" = {
...
EOF
        echo "  wrote LUKS device declaration: $LUKS_HW"

        # Keep the source repo coherent (same deliberate, non-destructive
        # pattern used for the hardware configuration).
        SOURCE_LUKS_HW="$REPO_DIR/hosts/$HOST/luks-configuration.nix"
        mkdir -p "$REPO_DIR/hosts/$HOST"
        if [[ -f "$SOURCE_LUKS_HW" ]]; then
            if cmp -s "$SOURCE_LUKS_HW" "$LUKS_HW"; then
                echo "  source: $SOURCE_LUKS_HW (unchanged)"
            elif yesno "Source repo already has $SOURCE_LUKS_HW. Overwrite it (a backup is kept)?"; then
                cp "$SOURCE_LUKS_HW" "$SOURCE_LUKS_HW.last-install.bak"
                cp "$LUKS_HW" "$SOURCE_LUKS_HW"
                echo "  source: $SOURCE_LUKS_HW (updated; backup at $SOURCE_LUKS_HW.last-install.bak)"
            else
                warn "source repository LUKS declaration left untouched."
            fi
        else
            cp "$LUKS_HW" "$SOURCE_LUKS_HW"
            echo "  source: $SOURCE_LUKS_HW (created)"
        fi
    else
        warn "could not read a UUID for $LUKS_PART — the LUKS device declaration will NOT be generated, and an encrypted root will not boot."
    fi
fi
