...
# ---------------------------------------------------------------------------
# LUKS (full-disk encryption) state.  Populated ONLY when the user opts into
# encryption during stage 1 (see 01-format.sh).  When empty, the installer
# behaves exactly as before — plain btrfs root, no LUKS anywhere.
#   . LUKS=1      encrypted root was requested/configured
#   . LUKS_PART   the raw partition that carries the LUKS container (e.g.
#                 /dev/nvme0n1p2) — this is the device referenced by
#                 boot.initrd.luks.devices.*.device
#   . LUKS_MAPPER the active dm-crypt name used for all root btrfs mounts
#                 (e.g. "nixos-crypt" -> /dev/mapper/nixos-crypt)
# The btrfs filesystem lives INSIDE the mapped device, so every root-side
# mkfs/mount must target $LUKS_MAPPER (i.e. /dev/mapper/$LUKS_MAPPER), never
# the raw partition.
# ---------------------------------------------------------------------------
LUKS=0
LUKS_PART=""
LUKS_MAPPER="nixos-crypt"
LUKS_UUID=""
...
# LUKS helpers.
#
# The root-side block device is either the raw partition (plain btrfs, LUKS=0)
# or the dm-crypt mapper (encrypted, LUKS=1).  All filesystem/mount operations
# on the root must go through root_blkdev(), never assume a raw partition.
# ---------------------------------------------------------------------------
...
# The block device that actually carries the root filesystem (btrfs).
root_blkdev() {
    if is_luks; then
        [[ -n "$LUKS_MAPPER" ]] || die "LUKS requested but LUKS_MAPPER is empty."
        printf '%s' "/dev/mapper/$LUKS_MAPPER"
    else
        printf '%s' "$ROOT_PART"
    fi
}

# Whether a device is already an open LUKS container (TYPE=crypto_LUKS).
# (Kept as a small helper; blkid returns crypto_LUKS for a luks-formatted
# partition whether or not it is currently decrypted.)
is_crypto_luks() {
    [[ "$(blkid -s TYPE -o value "$1" 2>/dev/null || true)" == "crypto_LUKS" ]]
}

# Open (decrypt) the LUKS container if it's not already open, prompting for the
# passphrase in the process.  Idempotent.
luks_open_if_needed() {
    is_luks || return 0
    [[ -n "$LUKS_PART" ]] || die "LUKS enabled but no LUKS partition recorded."
    local mapper="/dev/mapper/$LUKS_MAPPER"
    if [[ -e "$mapper" ]]; then
        info "LUKS container already open at $mapper."
        return 0
    fi
    info "Opening LUKS container $LUKS_PART as $mapper (type the passphrase)..."
    cryptsetup open "$LUKS_PART" "$LUKS_MAPPER"
}

# Close the LUKS container (nice-to-have; called after stage 1 exits).
luks_close_if_open() {
    is_luks || return 0
    local mapper="/dev/mapper/$LUKS_MAPPER"
    ...
    if [[ -e "$mapper" ]] && cryptsetup status "$LUKS_MAPPER" >/dev/null 2>&1; then
        cryptsetup close "$LUKS_MAPPER" 2>/dev/null || true
    fi
}

# Documented, manual TPM2 auto-unlock step the user runs AFTER first boot into
# the newly installed system (we cannot enroll an OS we haven't booted yet).
# The NixOS-side boot.initrd.luks.devices declaration is already generated by
# stage 2 into hosts/<host>/luks-configuration.nix (with allowDiscards and
# tpm2-device=auto), so the user only needs to run the enrollment command.
print_tpm_enroll_instructions() {
    is_luks || return 0
    [[ -n "$LUKS_PART" ]] || return 0
    local luks_uuid
    luks_uuid="$(uuid_of "$LUKS_PART")"
    echo
    section "POST-INSTALL: TPM2 AUTO-UNLOCK (OPTIONAL)"
    echo
    echo "The disk is now LUKS2-encrypted. It will prompt for the passphrase at"
    echo "every boot by default. To make it unlock hands-free via your TPM2"
    echo "chip (Secure Boot keeps PCR7 stable so this stays reliable), boot the"
    echo "NEW machine once, then run as root:"
    echo
    echo "  sudo systemd-cryptenroll --tpm2-device=auto --tpm2-pcrs=0+2+7 \"$LUKS_PART\""
    echo
    echo "No NixOS changes are needed: installer stage 2 already wrote the"
    echo "device declaration (allowDiscards + tpm2-device=auto) into"
    echo "hosts/$HOST/luks-configuration.nix. Your slot-0 passphrase remains the"
    echo "rescue key if the TPM/PCRs ever drift."
    if [[ -n "$luks_uuid" ]]; then
        echo
        echo "For reference, the generated device declaration is:"
        echo "  boot.initrd.luks.devices.\"$LUKS_MAPPER\".device = \"/dev/disk/by-uuid/$luks_uuid\";"
    fi
}